...
To import artwork on Android, open **Import / Export → Import Symbol Pack…**. The Activity retains the selected file across the security lock/unlock boundary; import starts only after mission data is unlocked. On iOS, open the marker symbol editor and choose **Import Symbol Pack…**. Select a TacMap symbol-pack JSON file. Choose **Custom Symbols**, select the artwork, enter the marker name and place it on the active layer. Both platforms provide offline search across pack, category and symbol names with image previews. Search ignores case and accents, and accepts multiple words (for example, `feuerwehr fuhr`). Artwork preserves its original colours and proportions. Your layer, pack and symbol names are user data and are not translated.
